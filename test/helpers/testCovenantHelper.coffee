...
covenantTestHelper = (_chai, util) ->
  ...
  Assertion.addMethod 'withValue', (value)->
    ...
    @assert value == @_obj._state?.value,
      "expected promise to be fulfilled with value #{inspect value}, but got #{inspect @_obj._state.value}",
      "expected promise not to be fulfilled with value #{value}"
  Assertion.addProperty 'rejected', ->
    new Assertion(@_obj).to.be.a.covenant
    @assert @_obj._state?.reason?,
  ...
  Assertion.addMethod 'withReason', (reason)->
    ...
    @assert reason == @_obj._state?.reason,
      "expected promise to be rejected with reason #{inspect reason}, but got #{inspect @_obj._state.reason}",
